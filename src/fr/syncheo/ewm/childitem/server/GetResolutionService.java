 ...
 */
public class GetResolutionService extends TeamRawService implements IGetStatesService {
    ...
    public void perform_GET(String uri, HttpServletRequest request, HttpServletResponse response) 
    	...
    	Identifier<IWorkflowAction> actionId = createResolutionIdentifier(actionIdStr);
    	if (workItemIdStr == null || actionId == null) {
		...
		try {
			IWorkItem wi = workItemService.findWorkItemById(workItemId, IWorkItem.DEFAULT_PROFILE, null);
			JSONArray resolutions = getPossibleResolution(wi, actionId, null);		
    ...
    public JSONArray getPossibleResolution(IWorkItem workItem, Identifier<IWorkflowAction> actionId, IProgressMonitor monitor)
		...
		Identifier<IResolution>[] resolutions = workflowInfo.getResolutionIds(actionId);
		
		for (Identifier<IResolution> resolution : resolutions) {
			JSONObject resObj = new JSONObject();
			resObj.put("id", workflowInfo.getIdentifier() + "/" + resolution.getStringIdentifier());
			resObj.put("name", workflowInfo.getResolutionName(resolution));
			rex.add(resObj);
		}
    ...
    public Identifier<IWorkflowAction> createResolutionIdentifier(String stringIdentifier) {
        if (stringIdentifier == null || stringIdentifier.isEmpty()) {
            return null;
        }

        Identifier<IWorkflowAction> actionId = Identifier.create(IWorkflowAction.class, stringIdentifier);
