...
/*            on(self.widget, "change", function(val){
		...
		getValues: function() {
			...
			var categoryUrl =  ccmUrl +
				"rpt/repository/workitem?fields=workitem/projectArea[contextId=" + self.paContextId.value + "]/enumerations[id=severity]/literals/(id|name)";
			...
			XHR.oslcXmlGetRequest(categoryUrl).then(
				function (data) {
					...
					var severityLiterals = enumerationNodes.map(function(node) {
					    ...
					    var enumId = ccmUrl + "oslc/enumerations/" + self.paContextId.value + "/severity/" + literalId; 
					...
					var storeData = severityLiterals.map(function(item){
